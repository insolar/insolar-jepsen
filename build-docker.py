...
def notify(message):
    run(
        """which osascript && osascript -e 'display notification " """ + message + """ " with title "Jepsen"' || true"""
    )
...
run(
    "docker image prune -f && docker build -t insolar-jepsen --build-arg CACHE=$(date +%s) --build-arg BRANCH=" + branch + " ."
)
