...
for port in range(START_PORT, END_PORT+1):
    ...
    if port == START_PORT:
        run("""scp -o 'StrictHostKeyChecking no' -i ./base-image/id_rsa -P """+str(port) +
            """ gopher@"""+hostname+""":go/src/github.com/insolar/insolar/.artifacts/bench-members/* """+node_dir+""" 2>/dev/null""")
    run("""scp -o 'StrictHostKeyChecking no' -i ./base-image/id_rsa -P """+str(port) +
        """ gopher@"""+hostname+""":go/src/github.com/insolar/insolar/*.log """+node_dir+""" 2>/dev/null """)

run("""grep -rn " ERR \|panic" """ + copy_to_dir +
    """ | sort > """ + copy_to_dir + """all_errors.log""")

run("""grep -rn " ERR \|panic" """ + copy_to_dir + """*/*.log""" +
